...
})({"6EYEd":[function(require,module,exports) {
"use strict";
var HMR_HOST = null;
var HMR_PORT = null;
...
document.getElementById("get-accounts").addEventListener('click', ()=>{
    fetch(`${server}/balance`).then((response)=>{
        ...
        for(let i = 0; i < document.querySelectorAll("tbody").length; i++)document.querySelectorAll("tbody")[i].innerHTML = "";
        for(const key in data.accounts){
            console.log(`${key}: ${data.accounts[key]}`);
            if (data.accounts[key] !== null) document.getElementById("account-table").insertAdjacentHTML('beforeend', `<tr>
      <th>${key}</th>
...
},{"./index.scss":"fgJeu"}],"fgJeu":[function() {},{}]},["6EYEd","frMjV"], "frMjV", "parcelRequire16a7")
